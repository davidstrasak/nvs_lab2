...
;Main ktery blika ledkami ve fazi nebo protifazi dle stavu tlacitka
MAIN									; MAIN navesti hlavni smycky programu											
				BL		RCC_CNF			; Volani podprogramu nastaveni hodinoveho systemu procesoru
										; tj. skok na adresu s navestim RCC_CNF a ulozeni navratove 
										; adresy do LR (Link Register)
				...
				LDR		R2, =GPIOC_BSRR ; bit set reset registr pro nastaveni ledek
				MOV R5, #0
					
				; my super clean code begins
				; R0 je registr casovace
LOOP
				CMP R5, #0
				BEQ ADDONE
				BNE REMOVEONE
...
INSYNC
				...
				BNE INSYNC_ON
INSYNC_ON		
				LDR 	R1, =(2_1 :SHL: 8 :OR: 2_1 :SHL: 9)
				B INSYNC_CONTINUE
INSYNC_OFF		
				LDR		R1, =(2_1 :SHL: 24 :OR: 2_1 :SHL: 25)
				B INSYNC_CONTINUE
INSYNC_CONTINUE
				STR		R1, [R2]
...
OUTOFSYNC
				...
				BNE OUTOFSYNC_ON
OUTOFSYNC_ON	
				LDR 	R1, =(2_1 :SHL: 8 :OR: 2_1 :SHL: 25)
				B OUTOFSYNC_CONTINUE
OUTOFSYNC_OFF	
				LDR		R1, =(2_1 :SHL: 24 :OR: 2_1 :SHL: 9)
				B OUTOFSYNC_CONTINUE
OUTOFSYNC_CONTINUE
				STR		R1, [R2]
				LDR		R0, =slowWaitTime
...
WAIT1			
				LDR		R2, =8000		; cislo abych ziskal cca 1ms dilci delay
WAIT			SUBS	R2, R2, #1		; narocnost operace asi 1 cyklus
				BNE		WAIT			; narocnost operace asi 1 cyklus
				SUBS	R0, R0, #1
				BNE		WAIT1
			
		        ALIGN 4     ; Force the next address to be a multiple of 4 (0xe4)
